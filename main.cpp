...
#define STB_IMAGE_IMPLEMENTATION // Define stb_image
#include <stb/stb_image.h> // stb_image include
#include <stb/stb_image_resize.h> // stb_image resize include
// Other includes
...
// Define faces for sphere's cube-map
std::vector<std::string> faces =
{
    "Humus-Cubemap/negx.jpg",
    "Humus-Cubemap/negy.jpg",
    "Humus-Cubemap/negz.jpg",
    "Humus-Cubemap/posx.jpg",
    "Humus-Cubemap/posy.jpg",
    "Humus-Cubemap/posz.jpg"
};
...
GLuint loadHeightMap(const char* path) {
    ...
    // Load the height map image (grayscale image)
    int width, height, nrChannels;
...
int main() {
    ...
    // DEFINE TEXTURES HERE Project 10 --> NOTE FOR PROJECT 10
     ...
     // Set stb vertical flip
   // stbi_set_flip_vertically_on_load(true);
    GLuint cubemapTexture = loadCubemap(faces); // Load cubemap texture
    glActiveTexture(GL_TEXTURE0); // Activate texture
    glBindTexture(GL_TEXTURE_CUBE_MAP, cubemapTexture); // Bind texture
    
    GLuint heightMapTexture = loadHeightMap("Bump-Map.jpg");  // Load height map texture
    glActiveTexture(GL_TEXTURE3); // Activate texture
    glBindTexture(GL_TEXTURE_2D, heightMapTexture); // Bind texture

    GLuint normalMapTexture = loadTexture("NormalMap.png"); // Load normal map texture
    glActiveTexture(GL_TEXTURE1); // Activate texture
    glBindTexture(GL_TEXTURE_2D, normalMapTexture); // Bind texture
    
    GLuint diffuseMapTexture = loadTexture("Bump-Picture.jpg"); // Load diffuse map texture
    glActiveTexture(GL_TEXTURE2); // Activate texture
    glBindTexture(GL_TEXTURE_2D, diffuseMapTexture); // Bind texture
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glUniform1i(glGetUniformLocation(cubeShader.Program, "diffuseMap"), 2);
        glUniform1i(glGetUniformLocation(cubeShader.Program, "normalMap"), 1);
        glUniform1i(glGetUniformLocation(cubeShader.Program, "heightMap"), 3);
        // Pass to shaders
        ...
        // Get uniform location for normalMap
        GLint normalMapLoc = glGetUniformLocation(cylinderShader.Program, "normalMap");
        GLint diffuseMapLoc = glGetUniformLocation(cylinderShader.Program, "diffuseMap");
        ...
        glUniform1i(normalMapLoc, 1); // Pass normalMap to uniform
        glUniform1i(diffuseMapLoc, 2); // Pass diffuseMap to uniform
        glUniform3f(cylinderColorLoc, 0.1f, 1.0f, 0.0f); // Pass color to uniform
        glUniform3f(lightColorLoc, 1.0f, 1.0f, 1.0f); // Pass light color to uniform
        glUniform3f(lightPosLoc, lightPos.x, lightPos.y, lightPos.z); // Pass light position to uniform
        glUniform3f(viewPosLoc, camera.Position.x, camera.Position.y, camera.Position.z); // Pass camera position to uniform
        ...
        glUniform1i(glGetUniformLocation(sphereShader.Program, "skybox"), 0); // Set skybox to 0
